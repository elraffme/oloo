          ...
          <CardContent className="space-y-6">
            {currentStep === 1 && <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input id="occupation" value={formData.occupation} onChange={e => handleInputChange('occupation', e.target.value)} placeholder="Software Engineer" required />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <Input type="number" min="4" max="7" value={formData.heightFeet} onChange={e => handleInputChange('heightFeet', e.target.value)} placeholder="5" />
                        <Label className="text-xs text-muted-foreground">Feet</Label>
                      </div>
                      <div>
                        <Input type="number" min="0" max="11" value={formData.heightInches} onChange={e => handleInputChange('heightInches', e.target.value)} placeholder="8" />
            ...
            {currentStep === 2 && <div className="space-y-6">
                <div>
                  ...
                  <Textarea id="bio" value={formData.bio} onChange={e => handleInputChange('bio', e.target.value)} placeholder="Tell others about yourself, your passions, and what you're looking for..." rows={4} />
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {availableInterests.map(interest => <Badge key={interest} variant={formData.interests.includes(interest) ? "default" : "outline"} className="cursor-pointer" onClick={() => toggleInterest(interest)}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {availableLanguages.map(language => <Badge key={language} variant={formData.languages.includes(language) ? "default" : "outline"} className="cursor-pointer" onClick={() => toggleLanguage(language)}>
            ...
            <div className="flex justify-between pt-6">
              ...
              {currentStep < 2 ? <Button onClick={nextStep} disabled={currentStep === 1 && (!formData.displayName || !formData.age || !formData.occupation)}>
                  Continue
                </Button> : <Button onClick={nextStep} disabled={!formData.bio || !formData.relationshipGoals || formData.interests.length === 0 || formData.languages.length === 0 || isSubmitting}>
