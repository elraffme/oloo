      ...
      >
        ...
        <div className="relative px-8 pt-10 pb-8">
          ...
          <div className="grid grid-cols-2 gap-2 max-h-[320px] overflow-y-auto pr-1 custom-scrollbar">
            {LANGUAGES.map((language) => (
              <button
                ...
                className={`group flex items-center justify-between px-2.5 py-2 rounded-xl border transition-all duration-200 text-left ${
              ...
              >
                <div className="min-w-0">
                  <p className={`font-medium text-sm truncate ${
                    selectedLanguage === language.code ? 'text-white' : 'text-white/80'
                  }`}>
                    {language.nativeName}
                  </p>
                  <p className={`text-xs truncate ${
                    selectedLanguage === language.code ? 'text-white/60' : 'text-white/40'
                  }`}>
                    {language.name}
                  </p>
                </div>
                <div className={`flex-shrink-0 ml-2 w-5 h-5 rounded-full flex items-center justify-center transition-all duration-200 ${
                  selectedLanguage === language.code 
          ...
          <Button 
            ...
            className="w-full mt-6 h-12 bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 text-primary-foreground font-medium rounded-xl transition-all duration-200 shadow-lg shadow-primary/20"
          >
            Continue
          </Button>
