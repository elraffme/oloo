-- Enhanced Payment Security: Field-level encryption and stricter access controls (Safe Update)
...
BEGIN
  ...
  CASE operation_type
    WHEN 'create' THEN
      ...
      -- Create payment with encrypted sensitive data if encryption fields exist
      IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'payment_intents' 
        AND column_name = 'encrypted_customer_id'
      ) THEN
      ...
      ELSE
        -- Fallback for tables without encryption columns
        INSERT INTO public.payment_intents (
          id,
          user_id,
          customer_id,
      ...
      FROM public.payment_intents WHERE id = payment_intent_id;
    ...
    WHEN 'update_status' THEN
      ...
      SET 
        ...
        updated_at = now(),
        security_flags = CASE 
          WHEN security_flags IS NOT NULL THEN 
            security_flags || jsonb_build_object('last_status_update', now())
          ELSE 
    ...
    WHEN 'webhook_update' THEN
      ...
      SET 
        ...
        security_flags = CASE 
          WHEN security_flags IS NOT NULL THEN 
            security_flags || jsonb_build_object('webhook_updated', now())
          ELSE 
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'protect_payment_encryption_trigger'
  ) THEN
