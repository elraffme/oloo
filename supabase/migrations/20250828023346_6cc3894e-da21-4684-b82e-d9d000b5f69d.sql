-- Fix Service Account Token Balance Security Vulnerability (Part 2)
-- Drop existing policies first, then create secure ones

-- Drop all existing token transaction policies
DROP POLICY IF EXISTS "Authenticated users can view own token transactions" ON public.token_transactions;
...
-- Create new restrictive RLS policies for token transactions
CREATE POLICY "Users can view own transactions only"
ON public.token_transactions
FOR SELECT
USING (auth.uid() = user_id AND auth.uid() IS NOT NULL);

CREATE POLICY "Secure token transactions via validated function only"
ON public.token_transactions
FOR INSERT
WITH CHECK (
  -- Only allow inserts through secure function or limited direct user rewards
  (
    -- Service operations through secure function (has security_validated metadata)
    (metadata->>'security_validated')::boolean = true
  ) OR (
    -- Direct user operations (gift receiving, etc.) - very limited scope
    auth.uid() = user_id 
    AND delta > 0 
    AND delta <= 100 
...
CREATE POLICY "No direct token transaction updates allowed"
ON public.token_transactions
FOR UPDATE
USING (false);

CREATE POLICY "No direct token transaction deletes allowed"  
ON public.token_transactions
FOR DELETE
USING (false);
...
-- Apply audit trigger
DROP TRIGGER IF EXISTS token_transaction_audit_trigger ON public.token_transactions;
...
ALTER TABLE public.token_transactions 
  DROP CONSTRAINT IF EXISTS valid_delta_range;
ALTER TABLE public.token_transactions 
  ADD CONSTRAINT valid_delta_range 
  CHECK (delta BETWEEN -10000 AND 10000);

ALTER TABLE public.token_transactions 
  DROP CONSTRAINT IF EXISTS valid_balance_range;
ALTER TABLE public.token_transactions 
