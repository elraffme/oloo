-- Enhanced Payment Security: Field-level encryption and stricter access controls
...
CREATE OR REPLACE FUNCTION public.decrypt_payment_field(encrypted_data text, plaintext_reference text, field_type text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  expected_hash := encode(
    digest(plaintext_reference || ':' || encryption_key, 'sha256'),
    'base64'
  );
...
BEGIN
  ...
  -- Get payment owner
  SELECT user_id, (access_restricted_until > now()) 
...
-- Update the secure_payment_operation function with enhanced security
...
BEGIN
  ...
  CASE operation_type
    ...
    WHEN 'update_status' THEN
      ...
      IF NOT validate_payment_access(payment_intent_id, 'update') THEN
...
-- Drop and recreate RLS policies with enhanced security
...
-- Enhanced RLS policies with stricter access controls
CREATE POLICY "enhanced_payment_no_direct_insert"
ON public.payment_intents
FOR INSERT
WITH CHECK (false);  -- All inserts must go through secure function

CREATE POLICY "enhanced_payment_no_direct_update"
ON public.payment_intents
FOR UPDATE
USING (false);  -- All updates must go through secure function

CREATE POLICY "enhanced_payment_secure_select"
ON public.payment_intents
FOR SELECT
USING (
  auth.uid() = user_id 
...
CREATE POLICY "enhanced_payment_no_delete"
ON public.payment_intents
FOR DELETE
USING (false);  -- Never allow direct deletes on payment data

-- Create trigger to prevent direct manipulation of encrypted fields
